...
Date: August 5, 2023
Description: The WhatABook program that will interact with the WhatABook SQL Database
"""
...
def show_books(_cursor):
    ...
    # use customer data and display the results
    for book in book:
...
def show_wishlist(_cursor, _user_id):
    """ query database for a list of books to be added to user wishlist """
...
def show_books_to_add(_cursor, _user_id):
    """ query database for a list of books not on the users wishlist """
...
for books in books_to_add:
    print ("    Book ID: {}\n   Book Name:{}\n".format(books[0], book[1]))

def add_book_to_wishlist(_cursor, _user_id, book_id):
    ...
    try:
        ...
        user_selection = show_menu() # showing main menu
        ...
        while user_selection != 4:
            ...
            # if the user selects option 2, call the show_locations method and display the configured location
            if user_selection == 2:
                show_locations (cursor)

            # if the user selects option 3, call the validate_user method to validate the entered user_id
            # call the show_account_menu() to show the account setting menu
            if user_selection == 3:
                my_user_id = validate_user()
                account_option = show_account_menu()

            # while account does not equal 3
            while account_option != 3:

                # if the user selects option 1, call the show_wishlist() method to show the current users
                # configured wishlist items
                if account_option == 1:
                    show_wishlist(cursor, my_user_id)

                # if the user selects option 2, call the show_books_to_add function to show the user
                # the books not currently in the users wishlist
                if account_option == 2:
                    ...
                    # add the selected book to the users wishlist
